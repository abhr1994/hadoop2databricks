...
# Source ID can be list of src_ids:x!

def dump_table_info(table_names=None):
    try:
        if table_names:
            table_list = [i+"_STG" for i in table_names.split(',')]
            docs = mongodb[CollectionName.TABLES].find({"$and": [{"table": {"$in": table_list}}, {
        ...
        with open('/tmp/tables_params_file.csv', mode='w') as param_file:
            ...
            for doc in docs:
                ...
                row = [str(doc["_id"]), doc['table'], doc['ingestion_configuration']['storage_format'],
                       ...
                       partition_cols, override_columns_str, doc["rowCount"]]
