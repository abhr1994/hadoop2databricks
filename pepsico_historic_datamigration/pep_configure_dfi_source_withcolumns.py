...
def make_source_ready(source_id):
    try:
        ...
        table_json_to_upd = {"meta_crawl_performed":True}
        mongodb["tables"].update_many({'source': src_id}, {'$set': table_json_to_upd})
...
def configure_table_group(configuration_obj,tablegrouptemplate_obj,cluster_id):
    out = []
...
def configure_table(configuration_obj,tabletemplate_obj,hive_schema):
    ...
    for table in configuration_obj["tables"]:
        try:
            ...
            """
            for key in ['source_relative_path','ingest_subdirectories', 'source_file_properties', 'exclude_filename_regex', 'include_filename_regex', 'target_relative_path']:
                if key == 'target_relative_path':
                    table_temp['configuration']['configuration'][key] = table['configuration']['configuration']['hdfs_relative_path']
                elif key == 'source_file_properties':
                    table_temp['configuration']['configuration'][key] = table['configuration']['configuration'][key]
                    del table_temp['configuration']['configuration'][key]['footer_rows_count']
                else:
                    table_temp['configuration']['configuration'][key] = table['configuration']['configuration'][key]
            """
            ...
            for column in table["configuration"]["columns"]:
                ...
                if not column["name"].startswith("ZIW"):
                    for key in ["name","origName","sqlType","targetSqlType"]:
...
def convert_csv_onprem_to_db(configuration_file_path,cluster_template):
    ...
    for i in range(len(configuration_obj["iw_mappings"])):
        if configuration_obj["iw_mappings"][i]["entity_type"] == "table":
            configuration_obj["iw_mappings"][i]["recommendation"]["table_name"] = configuration_obj["iw_mappings"][i]["recommendation"]["table_name"].rstrip("_STG")
    ...
    out = configure_table_group(configuration_obj,tablegrouptemplate_obj,cluster_id)
...
if existing_source_id_result and existing_source_id_result.get('entity_id') is not None:
    ...
    make_source_ready(str(source_id))
else:
...
configuration_obj = convert_csv_onprem_to_db(configuration_file_path,args['cluster_template'])
