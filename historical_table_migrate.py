...
parser.add_argument('--override_columns',required=False,help="List of columns to over-ride. Eg: '{'fullname': 'fullname2', 'ins__ts': 'ins_ts3', 'abhi': 'abhis'}'")

args = vars(parser.parse_args())
logging.basicConfig(filename='/opt/infoworks/temp/'+args['logfile'].strip(),filemode='a',level=logging.DEBUG,format='%(asctime)s - %(levelname)s - %(message)s')

try:
    db = DatabricksAPI(host=args['host'],token=args['token'])
    job_name = args['file_type'] + " to Delta - Migration"
    job_id = db.jobs.create_job(name=job_name,existing_cluster_id=args['cluster_id'],timeout_seconds=1800,spark_python_task={"python_file": "dbfs:/FileStore/tables/delta_conversion.py","parameters":None})
    #{'job_id': 47}
    if job_id.get('job_id',None):
        params = [args["src_dir"], args["delta_dir"], args["mode"], args["source_schema"],args["table_name"],args["src_type"],args["file_type"],args['partition_by'],args['override_columns']]
        ...
        while True:
            ...
            if (life_cycle_state is not None) and (life_cycle_state in ['SUCCESS', 'FAILED', 'TIMEDOUT', 'CANCELED','TERMINATING', 'TERMINATED', 'SKIPPED', 'INTERNAL_ERROR']):
