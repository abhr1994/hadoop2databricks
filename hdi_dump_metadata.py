    ...
    try:
        if src_id:
            ...
            docs = mongodb[CollectionName.TABLES].find({"$and": [{"state": "ready"},
                                                       ...
                                                       {"_id": 1, "source": 1, "table": 1, "hdfs_path": 1,
                                                        ...
                                                        "last_merged_timestamp": 1, "columns": 1, "rowCount": 1})
        else:
            docs = mongodb[CollectionName.TABLES].find({"$and": [{"state": "ready"}, {
        ...
        with open('/tmp/tables_params_file.csv', mode='w') as param_file:
            ...
            file_writer.writerow(
                ['table_id', 'table_name', 'storage_format', 'src_dir', 'source_id', 'last_ingested_cdc_value',
                 'last_merged_timestamp', 'sync_type', 'partition_column', 'override_columns', 'rowcount'])
            for doc in docs:
                ...
                b = partition_cols.encode("UTF-8")
                partition_cols_encoded = base64.b32encode(b)
                ...
                b = override_columns_str.encode("UTF-8")
                override_columns_encoded = base64.b32encode(b)
                row = [str(doc["_id"]), doc['table'], doc['ingestion_configuration']['storage_format'],
                       doc["hdfs_path"], str(doc['source']), doc['last_ingested_cdc_value'],
                       str(doc['last_merged_timestamp']), doc['ingestion_configuration']['sync_type'],
                       partition_cols_encoded.decode("UTF-8"), override_columns_encoded.decode("UTF-8"), doc["rowCount"]]
                if not src_id:
    ...
    try:
        with open('/tmp/source_params_file.csv', mode='w') as src_dump_file:
            ...
            for item in src_ids:
                ...
                doc = mongodb[CollectionName.SOURCES].find_one({"_id": src_id},
                                                               {"name": 1, "hive_schema": 1, "sourceType": 1,
                                                                "sourceSubtype": 1})
